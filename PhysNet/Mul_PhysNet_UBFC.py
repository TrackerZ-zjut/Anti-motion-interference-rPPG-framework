...
if __name__ == '__main__':
    ...
    modelDir = 'G:\\HWR\\ZMH\\Mul\\weight_150\\PhysNet_UBFC_150_3_epoch03.pth'  # trained model
    ...
    for iSub in range(1, nSub + 1):
        ...
        subID = 'subject' + str(iSub)
        vidDir = 'G:\\HWR\\Multi-scale rPPG\\Result\\UBFC_pictures\\' + subID
        NewResultDir = 'G:\\HWR\\ZMH\\Result\\UBFC_DATASET\\DATASET_2\\' + subID
        ResultDir = 'G:\\HWR\\Multi-scale rPPG\\Result\\UBFC_DATASET\\DATASET_2\\' + subID
        roi_File = ResultDir + '/roi_facedetector.mat'  # Original ROI coordinates tracked by KLT algorithm
        file2Save = NewResultDir + '/newMulRPN_UBFC_0927_PhysNet3D.mat'
        ...
        for n in range(winLength, nImages + 1, stepSize):  # 150,225,300,375...
            print(subID + 'window: ' + str(n) + ', first selection...')
            ...
            win_pulseEst_25Max = win_pulseEst1[First_index_25_SNR + 1,
                                 :]  # first selection signals extracted by physnet
            ROI25_max[:, n - winLength + 1: n + 1] = ROI25[First_index_25_SNR + 1, :, 1:151]

            # 第二次筛选
            print(subID + 'window: ' + str(n) + ', second selection...')
            ...
            win_pulseEst_9_Weights_Max = win_pulseEst2[First_index_9_Weights + 1,
                                         :]  # second selection signals extracted by physnet
            ROI9_Weights_max[:, n - winLength + 1: n + 1] = ROI9[First_index_9_Weights + 1, :, 1:151]

            win_pulseEst3 = np.zeros((6, winLength))

            print(subID + 'window: ' + str(n) + ', third selection...')
            ...
            for iROI3 in range(1, 6):
                ...
                win_pulseEst3[iROI3, :] = ppg_est_5_i[0, :]
                Weights_5 = getVarWeight(win_pulseEst3, winLength)
                SNRs_5[0, iROI3 - 1] = get_SNR(win_pulseEst3[iROI3, :],
                                               PUREfps)  # 5 multi scale signals extracted by physnet
