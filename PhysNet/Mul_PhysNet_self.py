...
def get_SNR(p, fps):  # define SNR
    ...
    freqRangeComp = ((freqPulse > 0) & (freqPulse < 0.8)) | (
                (freqPulse > 3.5) & (freqPulse < 15))  # noise range 0-0.8 Hz, 3.5-15Hz1
...
if __name__ == '__main__':
    ...
    modelDir = 'G:/HWR/ZMH/Mul/weight_150/PhysNet_Self_150_3_epoch03.pth'  # trained model
    ...
    for iSub in range(1, nSub + 1):
        for iVersion in range(1, nVersion + 1):
            ...
            vidDir = 'G:\\HWR\\Multi-scale rPPG\\Result\\self rPPG_pictures\\' + subID
            NewResultDir = 'G:\\HWR\\ZMH\\Result\\self rPPG\\' + subID
            ResultDir = 'G:\\HWR\\Multi-scale rPPG\\Result\\self rPPG\\' + subID
            roi_File = ResultDir + '/roi_facedetector.mat'  # Original ROI coordinates tracked by KLT algorithm
            file2Save = NewResultDir + '/newMulRPN_0927_Self_PhysNet3D.mat'
            ...
            PulseEst_Stg2_GS = np.zeros((1, nImages + 1))
            PulseEst_Stg2_Uni = np.zeros((1, nImages + 1))
            PulseEst_Stg2_W = np.zeros((1, nImages + 1))
            PulseEst_GS = np.zeros((1, nImages + 1))
            PulseEst_Exp = np.zeros((1, nImages + 1))
            PulseEst_Uni = np.zeros((1, nImages + 1))
            PulseEst_Var = np.zeros((1, nImages + 1))
            PulseEst_25Max = np.zeros((1, nImages + 1))
            PulseEst_9_Weights_Max = np.zeros((1, nImages + 1))

            ROI25 = np.zeros((25 + 1, 4, 150 + 1), dtype=np.int32)
            videoRPN25_1 = np.zeros((150 + 1, targHeight, targWidth, 3), dtype=np.float32)
            videoRPN25_2 = np.zeros((150 + 1, targHeight, targWidth, 3), dtype=np.float32)
            videoRPN25_3 = np.zeros((150 + 1, targHeight, targWidth, 3), dtype=np.float32)
            videoRPN25_4 = np.zeros((150 + 1, targHeight, targWidth, 3), dtype=np.float32)
            videoRPN25_5 = np.zeros((150 + 1, targHeight, targWidth, 3), dtype=np.float32)
            videoRPN25_6 = np.zeros((150 + 1, targHeight, targWidth, 3), dtype=np.float32)
            videoRPN25_7 = np.zeros((150 + 1, targHeight, targWidth, 3), dtype=np.float32)
            videoRPN25_8 = np.zeros((150 + 1, targHeight, targWidth, 3), dtype=np.float32)
            videoRPN25_9 = np.zeros((150 + 1, targHeight, targWidth, 3), dtype=np.float32)
            videoRPN25_10 = np.zeros((150 + 1, targHeight, targWidth, 3), dtype=np.float32)
            videoRPN25_11 = np.zeros((150 + 1, targHeight, targWidth, 3), dtype=np.float32)
            videoRPN25_12 = np.zeros((150 + 1, targHeight, targWidth, 3), dtype=np.float32)
            videoRPN25_13 = np.zeros((150 + 1, targHeight, targWidth, 3), dtype=np.float32)
            videoRPN25_14 = np.zeros((150 + 1, targHeight, targWidth, 3), dtype=np.float32)
            videoRPN25_15 = np.zeros((150 + 1, targHeight, targWidth, 3), dtype=np.float32)
            videoRPN25_16 = np.zeros((150 + 1, targHeight, targWidth, 3), dtype=np.float32)
            videoRPN25_17 = np.zeros((150 + 1, targHeight, targWidth, 3), dtype=np.float32)
            videoRPN25_18 = np.zeros((150 + 1, targHeight, targWidth, 3), dtype=np.float32)
            videoRPN25_19 = np.zeros((150 + 1, targHeight, targWidth, 3), dtype=np.float32)
            videoRPN25_20 = np.zeros((150 + 1, targHeight, targWidth, 3), dtype=np.float32)
            videoRPN25_21 = np.zeros((150 + 1, targHeight, targWidth, 3), dtype=np.float32)
            videoRPN25_22 = np.zeros((150 + 1, targHeight, targWidth, 3), dtype=np.float32)
            videoRPN25_23 = np.zeros((150 + 1, targHeight, targWidth, 3), dtype=np.float32)
            videoRPN25_24 = np.zeros((150 + 1, targHeight, targWidth, 3), dtype=np.float32)
            videoRPN25_25 = np.zeros((150 + 1, targHeight, targWidth, 3), dtype=np.float32)

            ROI9 = np.zeros((9 + 1, 4, 150 + 1), dtype=np.int32)
            videoRPN9_1 = np.zeros((150 + 1, targHeight, targWidth, 3), dtype=np.float32)
            videoRPN9_2 = np.zeros((150 + 1, targHeight, targWidth, 3), dtype=np.float32)
            videoRPN9_3 = np.zeros((150 + 1, targHeight, targWidth, 3), dtype=np.float32)
            videoRPN9_4 = np.zeros((150 + 1, targHeight, targWidth, 3), dtype=np.float32)
            videoRPN9_5 = np.zeros((150 + 1, targHeight, targWidth, 3), dtype=np.float32)
            videoRPN9_6 = np.zeros((150 + 1, targHeight, targWidth, 3), dtype=np.float32)
            videoRPN9_7 = np.zeros((150 + 1, targHeight, targWidth, 3), dtype=np.float32)
            videoRPN9_8 = np.zeros((150 + 1, targHeight, targWidth, 3), dtype=np.float32)
            videoRPN9_9 = np.zeros((150 + 1, targHeight, targWidth, 3), dtype=np.float32)

            ROI5 = np.zeros((5 + 1, 4, 150 + 1), dtype=np.int32)
            videoRPN5_1 = np.zeros((150 + 1, targHeight, targWidth, 3), dtype=np.float32)
            videoRPN5_2 = np.zeros((150 + 1, targHeight, targWidth, 3), dtype=np.float32)
            videoRPN5_3 = np.zeros((150 + 1, targHeight, targWidth, 3), dtype=np.float32)
            videoRPN5_4 = np.zeros((150 + 1, targHeight, targWidth, 3), dtype=np.float32)
            videoRPN5_5 = np.zeros((150 + 1, targHeight, targWidth, 3), dtype=np.float32)

            for n in range(winLength, nImages + 1, stepSize):  # 150,225,300,375...
                print(subID + ', n: ' + str(n) + ', collecting images...')
                for iImage in range(n - winLength + 1, n + 1):  # 1:150,  76:225,  151:300 ...

                    imageName = vidDir + '/' + str(iImage) + '.png'
                    currImage = cv2.imread(imageName)
                    ...
                    r_imgcrop1 = cv2.resize(imgcrop1, (targWidth, targHeight))
                    r_imgcrop2 = cv2.resize(imgcrop2, (targWidth, targHeight))
                    r_imgcrop3 = cv2.resize(imgcrop3, (targWidth, targHeight))
                    r_imgcrop4 = cv2.resize(imgcrop4, (targWidth, targHeight))
                    r_imgcrop5 = cv2.resize(imgcrop5, (targWidth, targHeight))
                    r_imgcrop6 = cv2.resize(imgcrop6, (targWidth, targHeight))
                    r_imgcrop7 = cv2.resize(imgcrop7, (targWidth, targHeight))
                    r_imgcrop8 = cv2.resize(imgcrop8, (targWidth, targHeight))
                    r_imgcrop9 = cv2.resize(imgcrop9, (targWidth, targHeight))
                    r_imgcrop10 = cv2.resize(imgcrop10, (targWidth, targHeight))
                    r_imgcrop11 = cv2.resize(imgcrop11, (targWidth, targHeight))
                    r_imgcrop12 = cv2.resize(imgcrop12, (targWidth, targHeight))
                    r_imgcrop13 = cv2.resize(imgcrop13, (targWidth, targHeight))
                    r_imgcrop14 = cv2.resize(imgcrop14, (targWidth, targHeight))
                    r_imgcrop15 = cv2.resize(imgcrop15, (targWidth, targHeight))
                    r_imgcrop16 = cv2.resize(imgcrop16, (targWidth, targHeight))
                    r_imgcrop17 = cv2.resize(imgcrop17, (targWidth, targHeight))
                    r_imgcrop18 = cv2.resize(imgcrop18, (targWidth, targHeight))
                    r_imgcrop19 = cv2.resize(imgcrop19, (targWidth, targHeight))
                    r_imgcrop20 = cv2.resize(imgcrop20, (targWidth, targHeight))
                    r_imgcrop21 = cv2.resize(imgcrop21, (targWidth, targHeight))
                    r_imgcrop22 = cv2.resize(imgcrop22, (targWidth, targHeight))
                    r_imgcrop23 = cv2.resize(imgcrop23, (targWidth, targHeight))
                    r_imgcrop24 = cv2.resize(imgcrop24, (targWidth, targHeight))
                    r_imgcrop25 = cv2.resize(imgcrop25, (targWidth, targHeight))
                    ...
                    videoRPN25_150_n = np.zeros((25 + 1, 150 + 1, targHeight, targWidth, 3), dtype=np.float32)
                ...
                # first selection
                print(subID + ', n: ' + str(n) + ', first selection...')
                win_pulseEst1 = np.zeros((25 + 1, winLength))
                for iROI1 in range(1, 26):
                    videoRPN25_150_i = np.zeros((150, targHeight, targWidth, 3))
                    ...
                    SNRs_25[0, iROI1 - 1] = get_SNR(win_pulseEst1[iROI1, :], PUREfps)  # get 25 defined SNRs
                ...
                print(subID + ', n: ' + str(n) + ', second selection...')
                for iImage2 in range(n - winLength + 1, n + 1):  # 1:150,  76:225,  151:300 ...
                    imageName2 = vidDir + '/' + str(iImage2) + '.png'
                    currImage2 = cv2.imread(imageName2)

                    k_150_2 = math.floor(iImage2 / winLength)
                    imageName_150_2 = iImage2 - k_150_2 * winLength
                    if imageName_150_2 == 0:
                        imageName_150_2 = 150
                    ...
                    imgcrop9ROI_1 = currImage2[y_91:y_91 + h_91, x_91:x_91 + w_91]
                    imgcrop9ROI_2 = currImage2[y_92:y_92 + h_92, x_92:x_92 + w_92]
                    imgcrop9ROI_3 = currImage2[y_93:y_93 + h_93, x_93:x_93 + w_93]
                    imgcrop9ROI_4 = currImage2[y_94:y_94 + h_94, x_94:x_94 + w_94]
                    imgcrop9ROI_5 = currImage2[y_95:y_95 + h_95, x_95:x_95 + w_95]
                    imgcrop9ROI_6 = currImage2[y_96:y_96 + h_96, x_96:x_96 + w_96]
                    imgcrop9ROI_7 = currImage2[y_97:y_97 + h_97, x_97:x_97 + w_97]
                    imgcrop9ROI_8 = currImage2[y_98:y_98 + h_98, x_98:x_98 + w_98]
                    imgcrop9ROI_9 = currImage2[y_99:y_99 + h_99, x_99:x_99 + w_99]

                    r_imgcrop9ROI_1 = cv2.resize(imgcrop9ROI_1, (targWidth, targHeight))
                    r_imgcrop9ROI_2 = cv2.resize(imgcrop9ROI_2, (targWidth, targHeight))
                    r_imgcrop9ROI_3 = cv2.resize(imgcrop9ROI_3, (targWidth, targHeight))
                    r_imgcrop9ROI_4 = cv2.resize(imgcrop9ROI_4, (targWidth, targHeight))
                    r_imgcrop9ROI_5 = cv2.resize(imgcrop9ROI_5, (targWidth, targHeight))
                    r_imgcrop9ROI_6 = cv2.resize(imgcrop9ROI_6, (targWidth, targHeight))
                    r_imgcrop9ROI_7 = cv2.resize(imgcrop9ROI_7, (targWidth, targHeight))
                    r_imgcrop9ROI_8 = cv2.resize(imgcrop9ROI_8, (targWidth, targHeight))
                    r_imgcrop9ROI_9 = cv2.resize(imgcrop9ROI_9, (targWidth, targHeight))
                    ...
                    videoRPN9_150_n = np.zeros((9 + 1, 150 + 1, targHeight, targWidth, 3), dtype=np.float32)
                ...
                for iROI2 in range(1, 10):
                    videoRPN9_150_i = np.zeros((150, targHeight, targWidth, 3))
                ...
                print(subID + ', n: ' + str(n) + ', third selection...')
                for iImage3 in range(n - winLength + 1, n + 1):  # 1:150,76:225,151:300...
                    imageName3 = vidDir + '/' + str(iImage3) + '.png'
                    currImage3 = cv2.imread(imageName3)

                    k_150_3 = math.floor(iImage3 / winLength)
                    imageName_150_3 = iImage3 - k_150_3 * winLength

                    if imageName_150_3 == 0:
                        imageName_150_3 = 150
                    ...
                    r_imgcrop5ROI_1 = cv2.resize(imgcrop5ROI_1, (targWidth, targHeight))
                    r_imgcrop5ROI_2 = cv2.resize(imgcrop5ROI_2, (targWidth, targHeight))
                    r_imgcrop5ROI_3 = cv2.resize(imgcrop5ROI_3, (targWidth, targHeight))
                    r_imgcrop5ROI_4 = cv2.resize(imgcrop5ROI_4, (targWidth, targHeight))
                    r_imgcrop5ROI_5 = cv2.resize(imgcrop5ROI_5, (targWidth, targHeight))
                    ...
                    videoRPN5_150_n = np.zeros((9 + 1, 150 + 1, targHeight, targWidth, 3), dtype=np.float32)
                ...
                for iROI3 in range(1, 6):
                    videoRPN5_150_i = np.zeros((150, targHeight, targWidth, 3))
