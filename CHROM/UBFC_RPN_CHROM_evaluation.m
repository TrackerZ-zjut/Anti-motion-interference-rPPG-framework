...
for iVersion = 1
    ...
    for iSub = 1 :nSubject
        ...
        ground_truth = dlmread( real_pulse_File );%  load real waveform
        gt_pulse = ground_truth( 1, : );
        
        load(PulseEst_File);  %  load predicted waveform
        
        nor_ppgClipped = normalizeSignal(gt_pulse');
        HR_Real = instantPulseFFT(nor_ppgClipped,fps_Real,false);
        gtHR = HR_Real;   %  real HR
        
        %         pulseEst = PulseEst_GS;
        pulseEst = PulseEst_Uni;
        nor_PulseEst = normalizeSignal(pulseEst); %   predicted waveform
        HR_Est = instantPulseFFT(nor_PulseEst,fps_Est,false);  %   predicted HR
        
        % ensure same length
        minLen = min( length(gtHR) , length(HR_Est) );
        gtHR = gtHR(1:minLen);
        HR_Est = HR_Est(1:minLen);
